...
def preprocess_plate_crop(bgr: np.ndarray) -> np.ndarray:
    ...
    min_side = max(
        16,
        _env_int("VEYTRA_OCR_MIN_SIDE", 48),
    )
...
def recognize_text(
    ...
    """Run PaddleOCR on a preprocessed BGR crop."""
    ...
    for text, conf in pairs:
        ...
        scored.append(
            (
                normalized,
                conf,
                is_valid_indian_plate(normalized),
            )
        )
...
def _finalize_confidence(
    ...
    if not valid:
        # Invalid format is retained internally but its
        # confidence is capped so downstream code can reject it.
        cap = _env_float(
            "VEYTRA_OCR_INVALID_CONF_CAP",
            0.45,
        )
...
def read_plate(
    plate_crop_image: Any,
) -> dict[str, Any]:
    """Recognize one plate crop.

    Independent of DETECT / TRACK / MATCH.

    Invalid or very-low-confidence OCR is returned with
    plate=None so downstream modules do not treat it as
    a successfully recognized registration number.
    """
    ...
    try:
        ...
        # Reject weak/invalid reads.
        #
        # This specifically prevents outputs such as:
        # AP7 -> plate=None
        #
        # Strong valid reads such as:
        # AP10AR0658 -> kept
